...
async function runTest() {
  ...
  try {
    ...
    // Upload first avatar
    const avatarForm1 = new FormData();
    avatarForm1.append("avatar", new Blob([pngA], { type: "image/png" }), "avatar-a.png");
    await api(baseUrl, "POST", "/api/profile/avatar", { token: studentToken, formData: avatarForm1 });

    const avatarUrl1 = await getAvatarUrl(studentUserId);
    assert.ok(avatarUrl1, "First avatar URL should be set");
    await waitForObjectState(avatarUrl1, true);

    // Replace with second avatar
    const avatarForm2 = new FormData();
    avatarForm2.append("avatar", new Blob([pngB], { type: "image/png" }), "avatar-b.png");
    await api(baseUrl, "POST", "/api/profile/avatar", { token: studentToken, formData: avatarForm2 });

    const avatarUrl2 = await getAvatarUrl(studentUserId);
    assert.ok(avatarUrl2 && avatarUrl2 !== avatarUrl1, "Second avatar URL should replace first");
    await waitForObjectState(avatarUrl2, true);
    await waitForObjectState(avatarUrl1, false, 40000);

    // Delete avatar
    await api(baseUrl, "DELETE", "/api/profile/avatar", { token: studentToken });
    await waitForObjectState(avatarUrl2, false, 40000);

    console.log("PASS: Avatar replacement and deletion cleanup verified.");
  } finally {
    if (studentUserId) {
